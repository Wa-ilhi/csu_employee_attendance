...
$fetch_query = pg_query($connection, "SELECT MAX(id) AS id FROM tbl_employee");
$row = pg_fetch_row($fetch_query);
...
if (isset($_REQUEST['add-employee'])) {
    ...
    $insert_query = pg_query($connection, "INSERT INTO tbl_employee (first_name, last_name, username, emailid, password, dob, employee_id, joining_date, gender, phone, shift, department, status, role) 
    VALUES ('$first_name', '$last_name', '$username', '$emailid', '$pwd', '$dob', '$employee_id', '$joining_date', '$gender', '$phone', '$shift', '$department', '$status', '0')");

    if ($insert_query) {
        $msg = "Employee created successfully";
    } else {
        $msg = "Error!" . pg_last_error($connection);
    }
}
?>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="employee_id" value="<?php if (!empty($emp_id)) {
                                                                                                        echo 'EMP-' . $emp_id;
                                                                                                    } else {
                                                                                                        echo "EMP-1";
                                                                                                    } ?>" disabled>
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <select class="select" name="shift" required>
                                    ...
                                    <?php
                                    $fetch_query = pg_query($connection, "SELECT start_time, end_time FROM tbl_shift WHERE status=1");
                                    while ($shift = pg_fetch_array($fetch_query)) {
                                    ?>
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <select class="select" name="department" required>
                                    <option value="">Select</option>
                                    <?php
                                    $fetch_query = pg_query($connection, "SELECT department_name FROM tbl_department WHERE status=1");
                                    while ($dept = pg_fetch_array($fetch_query)) {
                                    ?>
